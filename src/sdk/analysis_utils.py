...
def calculate_snr_improvement(input_s, target_s, pred_s):
    s = target_s.detach().cpu().numpy()
    i = input_s.detach().cpu().numpy()
    p = pred_s.detach().cpu().numpy()
...
def run_sensitivity_sweep(
    noise_levels: list, frequencies_idx: list = None, epochs: int = 1
):
    ...
    for noise in noise_levels:
        ...
        with torch.no_grad():
            for x, y in loader:
                b = x.size(0)
                x_dev, h_dev = gk.process_batch(x, model.init_hidden(b))
